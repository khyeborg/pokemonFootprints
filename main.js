...
let memorizedArray = [1, 7, 9, 18, 19, 26, 31, 46, 52, 56, 63, 64, 69, 76, 81, 82, 83, 84, 111, 123, 127, 137, 138, 141, 143, 150, 151, 163, 165, 168,  177, 178, 186, 193, 198, 207, 214, 217, 224, 227, 229, 233, 237, 240, 246, 251, 252, 253, 254, 257, 260, 261, 262, 263, 264, 267, 269, 271, 273, 275, 283, 286, 288, 289, 293, 294, 295, 300, 301, 303, 305, 306, 308, 309, 310, 323, 328, 329, 330, 335, 341, 342, 345, 346, 348, 352, 356, 357, 359, 374, 375, 376, 377, 379, 380, 381, 382, 383, 392, 396, 400, 441, 447, 448, 450, 454, 460, 463, 469, 475, 483, 484, 485, 486, 487, 496, 503, 525, 526, 536, 537, 549, 555, 556, 565, 580, 581, 598, 612, 614, 623, 633, 634, 643, 647];
...
for (let i = 0; i < 650; i++) {
  ...
  if (badNumArray.includes(i + 1) == false && genericArray.includes(i + 1) == false && memorizedArray.includes(i + 1) == false && birdsArray.includes(i + 1) == false && holesArray.includes(i + 1) == false) {
...
for (let i = 0; i < 650; i++) {
  ...
  if (badNumArray.includes(i + 1) == false && genericArray.includes(i + 1)) {
    ...
    if (name == "porygon-z") { name = "porygonz"; }
...
for (let i = 0; i < 650; i++) {
  ...
  // if the pokemon has a valid footprint
  if (badNumArray.includes(i + 1) == false && birdsArray.includes(i + 1)) {
    let name = pokedex[i].name.english.toLowerCase();
    let nameLength = name.length;
...
for (let i = 0; i < 650; i++) {
  ...
  // if the pokemon has a valid footprint
  if (badNumArray.includes(i + 1) == false && holesArray.includes(i + 1)) {
    let name = pokedex[i].name.english.toLowerCase();
    let nameLength = name.length;
